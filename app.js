...
let internacional = [];  // aba internacional
...
function parseWorkbook(wb) {
  ...
  const intlIdx = sheetNames.findIndex(n => n.includes('intern') || n.includes('internacional'));
...
function renderUSALic() {
  // Licenciados EUA: Origem preenchida (qualquer valor não vazio)
  let data = licenciados.filter(l => l.isUSA || (l.origem && l.origem.trim() !== ''));
  ...
  // Agrega por estado americano — usa usaState ou uf
  const byState = {};
  data.forEach(l => {
    const s = l.usaState || l.uf || 'XX';
